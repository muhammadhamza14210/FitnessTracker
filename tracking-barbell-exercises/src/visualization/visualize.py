...
for label in labels:
    for participant in participants:
        subset = df.query(f"label == '{label}'").query(f"participant == '{participant}'").reset_index()
        if len(subset) > 0:
            fig,ax = plt.subplots()
...
#Loop over combinations and export
for label in labels:
    for participant in participants:
        ...
        if len(combined_plot_df) > 0:
            ...
            #Saving image by label and participant
